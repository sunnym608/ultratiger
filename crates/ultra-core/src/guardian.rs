...
use crate::models::{GuardianStatusResponse, PreflightRequest, PreflightResponse};
...
impl Guardian {
    ...
    pub fn status(&self) -> GuardianStatusResponse {
        GuardianStatusResponse {
            total_spent_today_usd: self.total_spent_today_usd,
            daily_budget_limit_usd: self.config.daily_budget_limit_usd,
            preflight_authorize_threshold_usd: self.config.preflight_authorize_threshold_usd,
            keys_revoked: self.keys_revoked,
        }
    }

    pub fn manual_reset(&mut self) {
        self.total_spent_today_usd = 0.0;
        self.keys_revoked = false;
    }
...
mod tests {
    ...
    #[test]
    fn manual_reset_clears_spend_and_restores_keys() {
        let mut guardian = Guardian::new(AppConfig {
            daily_budget_limit_usd: 1.0,
            preflight_authorize_threshold_usd: 0.10,
        });
        ...
        assert_eq!(guardian.total_spent_today_usd(), 0.0);
        assert!(!guardian.keys_revoked());
    }
}
