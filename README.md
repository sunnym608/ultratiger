...
## 24/7 Autonomy Roadmap and Ops Toolkit

To align implementation with the "true 24/7 autonomous" target, the repository now includes:

- Roadmap and end-to-end user stories: `docs/ROADMAP_24_7_AUTONOMOUS.md`
- Incident response runbook: `deployment/ops/incident_runbook.md`
- SLO/error budget policy: `deployment/ops/slo_error_budget.md`
- Lifecycle scripts for operators:
  - `scripts/install_ultratiger.sh <version>`
  - `scripts/upgrade_ultratiger.sh <new_version>`
  - `scripts/rollback_ultratiger.sh <previous_version>`
  - `scripts/backup_ultratiger_db.sh [db_path] [backup_dir]`

These assets define and operationalize the required exit criteria:
reliability (99.9% uptime), autonomy (continuous workflows + retries/dead-letter replay),
safety (Guardian + approvals + policy gating), operability (one-command lifecycle + runbooks),
and UX guidance (onboarding, flow-builder, notifications, explainability).
